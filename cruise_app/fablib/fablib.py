...
from fabric.api import (abort, env, get, hide, hosts, lcd, local, put, roles,
                        run, runs_once, settings, sudo, task)
...
def extract(d, keys):
    """Return a subset of a dictionary based on list of keys"""
...
def chput(local_path=None, remote_path=None, user=None, group=None,
          mode=None, use_sudo=True, mirror_local_mode=False, check=False):
...
def rsync(local_path, remote_path, exclude=None):
    ...
    exclude.extend(['*.egg-info', '*.pyc', '.git', '.gitignore',
                    '.gitmodules', '.DS_Store', '/build/', '/dist/'])
    with hide('running'):
...
@task
def adduser(user='ubuntu'):
    """Add user <ubuntu> with sudo rights"""
    with settings(hide('warnings'), warn_only=True):
        if run('id {}'.format(user)).return_code:
            sudo('useradd -m -s /bin/bash {}'.format(user))
    sudoer_file = "/etc/sudoers.d/{}".format(user)
    sudoer = "{}\tALL=(ALL) NOPASSWD: ALL\n".format(user)
    chput(StringIO(sudoer), sudoer_file,
        'root', 'root', mode=0o0440)

 
@task
def addkey(key_file='~/.ssh/id_rsa.pub', user='ubuntu'):
    """Add ssh key to remote user authorized_keys"""
    key_file = os.path.expanduser(key_file)
    auth_file = '/home/{}/.ssh/authorized_keys'.format(user)
    if not key_file.endswith('pub'):
        raise RuntimeWarning('Non-public part of key pair')
    with open(key_file) as f:
        mkdir(os.path.dirname(auth_file), user, user,
            mode=0o0700)
        append(auth_file, f.read(), use_sudo=True)
        chown(auth_file, user, user)
        sudo('chmod 0600 {}'.format(auth_file))


@task
def targets():
    """Show available roles and host targets"""
    for role, roledef in sorted(env.roledefs.iteritems()):
        if callable(roledef):
            roledef = roledef()
        print("{0:12} : {1}".format(role, ", ".join(roledef)))
